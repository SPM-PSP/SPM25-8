...
long lastSendTime = 0; // 记录上次发送数据的时间
int beatAvg;
...
float smoothedBPM = 0; // Variable to hold the smoothed BPM

#define AVG_HEART_RATE_ADDR 0 // EEPROM 地址存储平均心率
#define AVG_SPO2_ADDR 4       // EEPROM 地址存储平均血氧
#define MEASURE_INTERVAL 5000 // 5秒测量间隔
#define PRINT_INTERVAL 5000   // 5秒打印间隔
...
// 从 EEPROM 读取数据
void readEEPROM() {
    avgHeartRate = EEPROM.get(AVG_HEART_RATE_ADDR, avgHeartRate);
    avgSpO2 = EEPROM.get(AVG_SPO2_ADDR, avgSpO2);
...
// 将数据写入 EEPROM
void writeEEPROM() {
    EEPROM.put(AVG_HEART_RATE_ADDR, avgHeartRate);
    EEPROM.put(AVG_SPO2_ADDR, avgSpO2);
}



void updateEMA(float newBPM) {
    smoothedBPM = alpha * newBPM + (1 - alpha) * smoothedBPM;
    beatAvg = (int)smoothedBPM; // Update the global beatAvg
}
...
void setup(void) {
  Serial.begin(9600);// 初始化串口用于蓝牙通信，波特率为9600

  // 进入AT模式并配置蓝牙模块
  Serial.println("AT"); // 发送AT命令以确认模块响应
  delay(1000);
  Serial.println("AT+BAUD4"); // 设置波特率为9600
  delay(1000);
  Serial.println("AT+NAMEHeartRate"); // 设置蓝牙名称
  delay(1000);
  Serial.println("AT+PIN1234"); // 设置PIN码
  delay(1000);
...
void loop()  {
    ...
    if (noww - lastMeasureTime >= MEASURE_INTERVAL) {
        lastMeasureTime = noww;

        // 读取传感器数据
        int heartRate = beatAvg; // 从现有代码获取心率
        int spO2 = SPO2f; // 从现有代码获取血氧

        if (heartRate >= 60) {
            measurementStarted = true;
            measureCount++;
            avgHeartRate = ((avgHeartRate * (measureCount - 1)) + heartRate) / measureCount;
            avgSpO2 = ((avgSpO2 * (measureCount - 1)) + spO2) / measureCount;
            writeEEPROM(); // 更新 EEPROM 中的平均值
        }
    }
    if (Serial.available()) {
    ...
    if (now - lastSendTime >= 5000) { // 每两秒发送一次数据
        int newHeartRate = beatAvg; // 获取心率
        int newSpO2 = SPO2f; // 获取血氧饱和度
        lastPrintTime = now;
        // 将心率和血氧饱和度数据发送到蓝牙模块
