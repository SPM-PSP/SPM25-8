    ...
    <el-row style="background-color: #FFFFFF;padding: 5px 0;border-radius: 5px;">
        ...
        <el-dialog :show-close="false" :visible.sync="dialogUserOperaion" width="50%">
            ...
            <div style="padding:0 20px;">
                ...
                <el-row>
                    <p>*标题</p>
                    <input style="font-size: 32px;line-height: 45px;" class="dialog-input" v-model="data.name"
                        placeholder="标题" />
                </el-row>
                <!-- 标签 -->
...
import baseURL from '@/config/baseURL.js'; // 加在开头
export default {
    components: { Editor },
    data() {
      return {
            uploadUrl: `${baseURL}/file/upload`, // 添加这行，集中管理
            userPwd: '',
            data: { cover: '' },
            filterText: '',
            currentPage: 1,
    ...
    methods: {
        channel() {
            this.dialogUserOperaion = false; // 关闭弹窗
            this.isOperation = false; //操作位置位
            this.data = {};// 数据清空
        },
        changeNewsTag(tagId) {
